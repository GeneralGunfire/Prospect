...
const TYPES: { value: SubmissionType; label: string; icon: React.ReactNode; desc: string }[] = [
  { value: 'school',  label: 'School',            icon: <GraduationCap className="w-5 h-5" />, desc: 'Primary, secondary or special needs' },
  { value: 'college', label: 'College',            icon: <BookOpen className="w-5 h-5" />,      desc: 'TVET, community or private college' },
  { value: 'job',     label: 'Job / Learnership',  icon: <Briefcase className="w-5 h-5" />,     desc: 'Apprenticeship, internship or opening' },
  { value: 'service', label: 'Support Service',    icon: <Heart className="w-5 h-5" />,         desc: 'Youth, health, food, skills or legal' },
];
...
const TYPE_META: Record<SubmissionType, { icon: React.ReactNode; label: string }> = {
  school:  { icon: <GraduationCap className="w-4 h-4" />, label: 'School' },
  college: { icon: <BookOpen className="w-4 h-4" />,      label: 'College' },
  job:     { icon: <Briefcase className="w-4 h-4" />,     label: 'Job' },
  service: { icon: <Heart className="w-4 h-4" />,         label: 'Service' },
};
...
function StatusBadge({ status }: { status: 'unverified' | 'verified' | 'pending' | 'approved' | 'rejected' }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-black border ${cls}`}>
      {icon}{label}
    </span>
  );
}
...
function SubmissionCard({ item, index }: { item: Submission; index: number }) {
  ...
  return (
    <motion.div
      ...
      className="bg-white rounded-xl border border-slate-100 p-5 transition-colors hover:border-slate-200"
    >
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mb-2">
            ...
            {item.category && (
              <span className="text-[14px] text-slate-400 bg-slate-50 border border-slate-100 px-2 py-0.5 rounded-lg">
                {item.category}
              </span>
            )}
          ...
          {item.description && (
            <p className="text-[14px] leading-relaxed text-slate-500 line-clamp-2 mb-2">
              {item.description}
            </p>
          )}
          ...
          {item.website_url && (
            <a
              ...
              className="inline-flex items-center gap-1.5 text-[11px] font-black uppercase tracking-widest text-slate-600 hover:text-slate-900 transition-colors"
            >
...
function CommunityImpactPage({ onNavigate }: { onNavigate: (page: AppPage) => void }) {
  ...
  async function loadMySubmissions() {
    ...
    setMyLoading(true);
    const data = await getUserSubmissions(form.email, undefined);
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="pt-24 pb-12 sm:pb-20 px-4 sm:px-6 max-w-5xl mx-auto">
        ...
        {/* Tabs */}
        <div className="p-1 bg-slate-100 rounded-xl flex gap-1 mb-6 sm:mb-8 w-full sm:w-fit overflow-x-auto">
          {(['discover', 'add', 'my-contributions'] as Tab[]).map(tab => {
            ...
            return (
              <button
                ...
                className={`px-4 py-2.5 min-h-11 rounded-lg text-[11px] font-black uppercase tracking-widest transition-all shrink-0 ${
                  activeTab === tab
        ...
        {activeTab === 'discover' && (
          <div>
            ...
            >
              <div>
                <p className="text-[11px] font-black uppercase tracking-widest text-slate-400 mb-1">Missing something?</p>
                <p className="text-white font-black text-base md:text-lg" style={{ letterSpacing: '-0.025em' }}>
                  Is your school or local opportunity here?
                </p>
                <p className="text-slate-400 text-[14px] leading-relaxed mt-1">Add it in under 60 seconds — no account needed.</p>
              </div>
              <button
                onClick={() => handleTabChange('add')}
                className="shrink-0 flex items-center gap-2 px-5 py-3 bg-white text-slate-900 rounded-xl font-black text-[11px] uppercase tracking-widest hover:bg-slate-100 transition-colors min-h-11"
              >
            ...
            {/* Stats Row */}
            <div className="rounded-xl border border-slate-100 overflow-hidden divide-y divide-slate-100 sm:divide-y-0 sm:flex mb-6 sm:mb-8">
              {[
                ...
                <div key={s.label} className="flex-1 p-4 text-center sm:border-r sm:last:border-r-0 border-slate-100">
            ...
            <div className="space-y-3 mb-6">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-4 py-2.5 text-[14px] bg-white border border-slate-200 rounded-xl focus:outline-none focus:border-slate-400 transition-all"
                />
                {searchQuery && (
              ...
              <div className="flex gap-2 overflow-x-auto pb-1">
                {(['all', 'school', 'college', 'job', 'service'] as const).map(t => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-[11px] font-black uppercase tracking-widest border transition-all shrink-0 ${
                      filterType === t
            ...
            {isLoading ? (
              ...
              <div className="text-center py-16">
                ...
                <p className="text-[15px] font-black text-slate-700 mb-1">No opportunities found</p>
                <p className="text-[14px] text-slate-400 mb-6">Be the first to add one in this area.</p>
                <button
                  onClick={() => handleTabChange('add')}
                  className="px-6 py-3 min-h-11 bg-slate-900 text-white rounded-xl font-black text-[11px] uppercase tracking-widest hover:bg-slate-800 transition-colors"
                >
                  Add an opportunity
                </button>
              </div>
        ...
        {activeTab === 'add' && (
          <div className="max-w-lg mx-auto">
            <AnimatePresence mode="wait">
              {/* Step 1: Type selection — divide-y list instead of grid cards */}
              {addStep === 'type' && (
                <motion.div
                  key="type"
                  ...
                  exit={{ opacity: 0, y: -10 }}
                >
                  <p className="text-[11px] font-black uppercase tracking-widest text-slate-400 mb-2">Step 1 of 2</p>
                  <h2 className="text-xl font-black text-slate-900 mb-1" style={{ letterSpacing: '-0.025em' }}>What are you adding?</h2>
                  ...
                  <div className="rounded-xl border border-slate-100 overflow-hidden divide-y divide-slate-100">
                    {TYPES.map(t => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-black text-[15px] text-slate-900">{t.label}</p>
                          <p className="text-[14px] text-slate-500 leading-snug">{t.desc}</p>
                        </div>
                        <ArrowRight className="w-4 h-4 text-slate-300 shrink-0 group-hover:text-slate-600 transition-colors" />
                      </button>
                    ))}
                  </div>
                </motion.div>
              ...
              {addStep === 'form' && selectedType && (
                ...
                >
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-slate-200 bg-slate-50 text-[11px] font-black uppercase tracking-widest text-slate-600">
                      {TYPE_META[selectedType].icon}
                      {TYPE_META[selectedType].label}
                    </div>
                  </div>

                  <p className="text-[11px] font-black uppercase tracking-widest text-slate-400 mb-2">Step 2 of 2</p>
                  <h2 className="text-xl font-black text-slate-900 mb-6" style={{ letterSpacing: '-0.025em' }}>Quick details</h2>
                  ...
                  <AnimatePresence>
                    {isDuplicateWarning && (
                      ...
                      >
                        <p className="text-[11px] font-black uppercase tracking-widest text-amber-800 mb-1">This might already exist</p>
                        <p className="text-[14px] text-amber-700">We found a similar entry for "{form.name}" in {form.city}. Submit anyway to add more details or a different location.</p>
                        <button
                          onClick={handleSubmit}
                          className="mt-3 text-[11px] font-black uppercase tracking-widest text-amber-800 underline hover:no-underline"
                        >
                          Submit anyway
                        </button>
                      </motion.div>
                  ...
                  <div className="space-y-4">
                    ...
                    <FormField label="Short description" hint="Optional — max 150 chars">
                      ...
                      />
                      <p className="text-right text-[14px] text-slate-400 mt-1">{form.description.length}/150</p>
                    </FormField>
                    ...
                    <div className="border-t border-slate-100 pt-4 mt-2">
                      <p className="text-[11px] font-black uppercase tracking-widest text-slate-400 mb-3">Your details</p>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      ...
                      <p className="text-[14px] text-slate-400 mt-2">Email is used for verification only, not displayed publicly.</p>
                    </div>
                    ...
                    <AnimatePresence>
                      {formError && (
                        <motion.p
                          ...
                          animate={{ opacity: 1 }}
                          exit={{ opacity: 0 }}
                          className="text-[14px] text-red-600 font-medium flex items-center gap-1.5"
                        >
                    ...
                    <button
                      ...
                      className="w-full py-4 bg-slate-900 text-white rounded-xl font-black text-[11px] uppercase tracking-widest hover:bg-slate-800 transition-colors disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                    >
                      {isSubmitting ? (
                        <>
              ...
              {addStep === 'success' && newSubmission && (
                ...
                >
                  <div className="w-16 h-16 bg-slate-100 border border-slate-200 rounded-xl flex items-center justify-center mx-auto mb-5">
                  ...
                  <p className="text-[14px] leading-relaxed text-slate-500 mb-8 max-w-xs mx-auto">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 justify-center">
                    <button
                      ...
                      className="px-6 py-3 min-h-11 border border-slate-200 text-slate-700 rounded-xl font-black text-[11px] uppercase tracking-widest hover:bg-slate-50 transition-colors"
                    >
                      View in Discover
                    </button>
                    <button
                      onClick={handleAddAnother}
                      className="px-6 py-3 min-h-11 bg-slate-900 text-white rounded-xl font-black text-[11px] uppercase tracking-widest hover:bg-slate-800 transition-colors flex items-center gap-2"
                    >
          ...
          <div>
            {!form.email ? (
              <div className="max-w-sm mx-auto text-center py-12">
                ...
                <h3 className="text-[15px] font-black text-slate-800 mb-2">Enter your email to see your submissions</h3>
                <p className="text-[14px] text-slate-500 mb-5">We track your contributions by email.</p>
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="px-4 py-2.5 min-h-11 bg-slate-900 text-white rounded-xl font-black text-[11px] uppercase tracking-widest hover:bg-slate-800 transition-colors"
                  >
                    View
                  </button>
                </div>
              ...
              <div className="text-center py-14">
                ...
                <p className="text-[15px] font-black text-slate-700 mb-1">No contributions yet</p>
                <p className="text-[14px] text-slate-400 mb-6">Add a school, college, job or service to your community.</p>
                <button
                  onClick={() => handleTabChange('add')}
                  className="px-6 py-3 min-h-11 bg-slate-900 text-white rounded-xl font-black text-[11px] uppercase tracking-widest hover:bg-slate-800 transition-colors"
                >
