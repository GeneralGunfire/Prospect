...
function GeneralLedgerPage({ user, onSignOut, onNavigate }: AuthedProps) {
  ...
  useEffect(() => {
    const load = async () => {
      const { data } = await supabase.from('study_progress').select('topic,mastery_level').eq('user_id', user.id).eq('subject', SUBJECT).eq('grade', GRADE).eq('topic', TOPIC_ID).single()
      if (!data) return
      const st: TopicStatus = data.mastery_level === 'mastered' ? 'mastered' : data.mastery_level === 'needs-practice' ? 'needs-practice' : 'not-started'
      setThisStatus(st)
    }
    load()
  }, [user.id])
...
export default withAuth(GeneralLedgerPage)
