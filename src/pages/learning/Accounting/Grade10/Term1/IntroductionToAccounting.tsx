          ...
          <div className="overflow-x-auto py-12 -mx-2 px-2">
            <div className="flex items-center justify-center gap-4 md:gap-6 relative min-w-max mx-auto">
              {step.math.map((char, i) => {
                ...
                return (<motion.div key={i} initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: i * 0.1 }} className="relative text-5xl font-mono font-black text-slate-900">{char}{bubble && <SpeechBubble text={bubble.text} pos={bubble.pos} />}</motion.div>)
              })}
            </div>
          </div>
        </motion.div>
...
const GuidedPracticeModule = ({ onComplete }: { onComplete: () => void }) => {
  ...
  return (
    <div className="space-y-3"><div className="bg-white border border-slate-200 rounded-[2.5rem] p-8 md:p-12 shadow-sm">
      <div className="flex items-center justify-between mb-6"><p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Step-by-Step Guide</p><p className="text-[10px] font-black uppercase tracking-widest text-blue-600">Step {stepIndex + 1} of {steps.length}</p></div>
      ...
      <div className="space-y-3 mb-8">{steps.slice(0, stepIndex + 1).map((s, i) => (
        <motion.div key={i} initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.25 }} className={`flex gap-5 p-4 rounded-2xl border ${i === stepIndex ? 'bg-blue-50 border-blue-200' : 'bg-slate-50 border-slate-100'}`}>
  ...
  return (
    <AnimatePresence><motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 backdrop-blur-sm p-0 sm:p-4" onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <motion.div initial={{ y: 60, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 60, opacity: 0 }} transition={{ type: 'spring', stiffness: 300, damping: 30 }} className="w-full sm:max-w-lg bg-white rounded-t-2xl sm:rounded-2xl shadow-lg flex flex-col overflow-hidden" style={{ maxHeight: '90dvh' }}>
        <div className="px-5 pt-5 pb-3 border-b border-slate-100 shrink-0"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-[10px] font-black uppercase tracking-widest text-blue-600 mb-1">Working Space</p><p className="text-sm font-black text-slate-900 leading-snug">{question}</p>{math && <div className="overflow-x-auto mt-1 -mx-1 px-1"><span className="whitespace-nowrap font-mono text-sm font-black text-blue-700">{math}</span></div>}</div><button onClick={onClose} aria-label="Close" className="shrink-0 w-11 h-11 flex items-center justify-center rounded-xl text-slate-400 hover:bg-slate-100 transition-colors"><X size={16} /></button></div></div>
...
const PracticeModule = ({ questions, onComplete }: { questions: Question[]; onComplete: (res: { correct: number; total: number }) => void }) => {
  ...
  const getOptionStyle = (i: number) => { if (!revealed) return selected === i ? 'border-blue-500 bg-blue-50' : 'border-slate-200 bg-white hover:border-blue-200 hover:bg-blue-50 cursor-pointer'; if (i === q.correctIndex) return 'border-emerald-500 bg-emerald-50 text-emerald-900'; if (i === selected) return 'border-rose-500 bg-rose-50 text-rose-900'; return 'border-slate-100 bg-white text-slate-300' }
  return (
    <>{scratchpadOpen && <ScratchpadModal question={q.question} math={q.math} storageKey={`${STORAGE_KEY_PREFIX}${current}`} onClose={() => setScratchpadOpen(false)} />}
      <div className="space-y-6"><div className="bg-white border border-slate-200 rounded-[2.5rem] p-8 md:p-12 shadow-sm">
        <div className="flex items-center justify-between mb-6"><p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Question {current + 1} of {questions.length}</p><div className="flex gap-1">{questions.map((_, i) => <div key={i} className={`w-8 h-1 rounded-full transition-all ${i <= current ? 'bg-blue-600' : 'bg-slate-100'}`} />)}</div></div>
        ...
        <div className="mt-8 flex justify-between items-center">
          {!revealed ? <button onClick={() => setHintVisible(true)} className="text-xs font-black text-amber-600 uppercase tracking-widest px-4 py-3 hover:bg-amber-50 rounded-xl transition-all">{hintVisible ? 'Hint Visible' : 'Need a Hint?'}</button> : <div />}
...
const FeedbackModule = ({ correct, total, onRetry, onPracticeMore, onContinue, hidePracticeMore = false }: { correct: number; total: number; onRetry: () => void; onPracticeMore: () => void; onContinue: () => void; hidePracticeMore?: boolean }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-[2.5rem] border border-slate-200 p-10 md:p-14 text-center space-y-8 shadow-sm">
      ...
      <div><p className={`text-[10px] font-black uppercase tracking-widest mb-2 ${mastered ? 'text-emerald-600' : 'text-slate-400'}`}>{mastered ? '✦ Mastered' : 'Keep Practising'}</p><h2 className="text-4xl font-black text-slate-900 tracking-tighter">Your Results</h2><p className="text-sm text-slate-400 font-semibold mt-1">{TOPIC.title}</p></div>
      <div className="flex flex-col items-center gap-3"><p className="text-6xl font-black text-slate-900">{correct} <span className="text-slate-300 text-4xl">/</span> {total}</p><div className="w-full max-w-xs h-2 bg-slate-100 rounded-full overflow-hidden"><motion.div initial={{ width: 0 }} animate={{ width: `${pct}%` }} transition={{ duration: 0.8, delay: 0.3 }} className={`h-full rounded-full ${mastered ? 'bg-emerald-500' : 'bg-blue-500'}`} /></div></div>
      <div className="flex flex-col gap-3 pt-2">
        <div className={`grid grid-cols-1 ${hidePracticeMore ? '' : 'sm:grid-cols-2'} gap-3`}><button onClick={onRetry} className="w-full py-5 bg-slate-100 text-slate-700 rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-slate-200 transition-all">Try Again</button>{!hidePracticeMore && <button onClick={onPracticeMore} className="w-full py-5 bg-blue-50 text-blue-700 border border-blue-200 rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-blue-100 transition-all flex items-center justify-center gap-3">Practice More <NotebookPen size={18} /></button>}</div>
...
async function loadTopicProgress(userId: string): Promise<TopicStatus> {
  const { data } = await supabase.from('study_progress').select('mastery_level').eq('user_id', userId).eq('subject', SUBJECT).eq('grade', GRADE).eq('topic', TOPIC_ID).single()
  if (!data) return 'not-started'; const m = data.mastery_level as string; if (m === 'mastered') return 'mastered'; if (m === 'needs_practice') return 'needs-practice'; return 'not-started'
}
async function saveTopicProgress(userId: string, status: TopicStatus, correct: number, total: number, attempts: number) {
  ...
  await supabase.from('study_progress').upsert({ user_id: userId, subject: SUBJECT, grade: GRADE, topic: TOPIC_ID, mastery_level: masteryLevel, last_attempt_score: `${correct}/${total}`, total_attempts: attempts, last_accessed: new Date().toISOString(), updated_at: new Date().toISOString() }, { onConflict: 'user_id,subject,grade,topic' })
}
async function loadNextTopicProgress(userId: string): Promise<TopicStatus> {
  const { data } = await supabase.from('study_progress').select('mastery_level').eq('user_id', userId).eq('subject', SUBJECT).eq('grade', GRADE).eq('topic', NEXT_TOPIC_ID).single()
  if (!data) return 'not-started'; const m = data.mastery_level as string; if (m === 'mastered') return 'mastered'; if (m === 'needs_practice') return 'needs-practice'; return 'not-started'
}

function IntroductionToAccountingPage({ user, onNavigate }: AuthedProps) {
  ...
  useEffect(() => { if (!user) return; loadTopicProgress(user.id).then(s => setStatus(s)); loadNextTopicProgress(user.id).then(s => setNextStatus(s)) }, [user])
  const saveProgress = async (newStatus: TopicStatus, res: { correct: number; total: number }) => { const n = attempts + 1; setAttempts(n); if (user) await saveTopicProgress(user.id, newStatus, res.correct, res.total, n) }
              ...
              <div className="space-y-4">
                <motion.div onClick={() => setView('interactive-lesson')} className="bg-white rounded-xl border border-slate-200 p-7 md:p-9 flex items-center justify-between gap-6 cursor-pointer hover:border-slate-300 hover:shadow-md transition-all group">
                  <div className="flex items-center gap-6">
                    <div className={`shrink-0 w-16 h-16 md:w-20 md:h-20 rounded-[1.25rem] flex items-center justify-center text-2xl font-black ${status === 'mastered' ? 'bg-emerald-500 text-white' : 'bg-slate-900 text-white shadow-md shadow-slate-900/10'}`}>{status === 'mastered' ? '✓' : '1'}</div>
                    <div>
                ...
                <motion.div onClick={() => onNavigate('learning-accounting-g10-t1-equation' as AppPage)} className="bg-white rounded-xl border border-slate-200 p-7 md:p-9 flex items-center justify-between gap-6 cursor-pointer hover:border-slate-300 hover:shadow-md transition-all group">
                  <div className="flex items-center gap-6">
                    <div className={`shrink-0 w-16 h-16 md:w-20 md:h-20 rounded-[1.25rem] flex items-center justify-center text-2xl font-black ${nextStatus === 'mastered' ? 'bg-emerald-500 text-white' : 'bg-slate-900 text-white shadow-md shadow-slate-900/10'}`}>{nextStatus === 'mastered' ? '✓' : '2'}</div>
                    <div>
            ...
            <div className="max-w-2xl mx-auto">
              ...
              <AnimatePresence mode="wait">
                <motion.div key={view} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.25 }}>
                  ...
                  {view === 'remediation' && (
                    <div className="space-y-6"><div className="bg-rose-50 border border-rose-200 rounded-xl p-8 flex gap-5 items-start"><AlertCircle className="text-rose-500 shrink-0 mt-0.5" size={24} /><div><p className="text-base font-black text-rose-900 uppercase tracking-tight mb-1">Let's Try Again</p><p className="text-rose-700 text-sm leading-relaxed">It's okay — let's work through two extra questions to build your confidence.</p></div></div>
...
export default withAuth(IntroductionToAccountingPage)
