  ...
  return (
    <div className="max-w-md mx-auto text-center space-y-6">
      <div className={`w-20 h-20 rounded-full flex items-center justify-center mx-auto ${mastered ? 'bg-emerald-100' : 'bg-amber-100'}`}>
        {mastered ? <Award className="w-10 h-10 text-emerald-600" /> : <AlertCircle className="w-10 h-10 text-amber-600" />}
      </div>
      ...
      <div className="flex flex-col gap-3">
        {mastered
          ? <button onClick={onContinue} className="flex items-center justify-center gap-2 px-6 py-3 bg-emerald-600 text-white rounded-xl hover:bg-emerald-700 transition-colors font-medium">Next: {nextTopicName} <ArrowRight className="w-4 h-4" /></button>
          : <>
...
function JournalsInAccountingPage({ user, onSignOut, onNavigate }: AuthedProps) {
  ...
  useEffect(() => {
    const load = async () => {
      const { data } = await supabase.from('study_progress').select('topic,mastery_level').eq('user_id', user.id).eq('subject', SUBJECT).eq('grade', GRADE).in('topic', [TOPIC_ID, NEXT_TOPIC_ID])
      if (!data) return
  ...
  const saveProgress = async (score: number, total: number) => {
    ...
    await supabase.from('study_progress').upsert({ user_id: user.id, subject: SUBJECT, grade: GRADE, topic: TOPIC_ID, mastery_level: mastery, last_attempt_score: Math.round((score / total) * 100), total_attempts: 1 }, { onConflict: 'user_id,subject,grade,topic' })
...
export default withAuth(JournalsInAccountingPage)
