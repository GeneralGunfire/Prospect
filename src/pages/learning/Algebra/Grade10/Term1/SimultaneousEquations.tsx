          ...
          {steps.slice(0, stepIndex + 1).map((s, i) => (
            <motion.div key={i} initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.25 }}
              ...
              <div className="min-w-0">
                ...
                <div className="inline-block px-4 py-2 bg-white border border-slate-200 rounded-xl text-lg font-mono font-black text-blue-600 shadow-sm">{s.math}</div>
              </div>
            </motion.div>
          ))}
        </div>
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white border border-slate-200 rounded-[2.5rem] p-8 md:p-12 shadow-sm">
        ...
        <p className="text-xl md:text-2xl font-black text-slate-900 mb-8 leading-snug">{q.question}</p>
...
const FeedbackModule = ({ correct, total, onRetry, onContinue }: { correct: number; total: number; onRetry: () => void; onContinue: () => void }) => {
