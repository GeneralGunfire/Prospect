                ...
                return (
                  <div key={i} className="relative">
                    ...
                    <button onClick={() => setActiveBubble(isActive ? null : token)}
                      className={`px-3 py-2 rounded-lg font-mono text-xs font-semibold transition-all ${bubble ? (isActive ? 'bg-blue-600 text-white shadow-md scale-105' : 'bg-blue-50 text-blue-700 border border-blue-200 hover:bg-blue-100') : 'bg-white text-slate-400 border border-slate-200 cursor-default'}`}>
                      {token}
                    </button>
                  </div>
                )
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {TOPIC.guidedItem.steps.map((s, i) => <button key={s.id} onClick={() => { setStepIdx(i); setRevealed(false) }} className={`shrink-0 px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${i === stepIdx ? 'bg-slate-900 text-white' : i < stepIdx ? 'bg-blue-100 text-blue-700' : 'bg-slate-100 text-slate-500'}`}>Step {s.id}</button>)}
      </div>
      <AnimatePresence mode="wait">
  ...
  return (
    <div className="max-w-md mx-auto text-center space-y-6">
      ...
      <div className="flex flex-col gap-3">
        <button onClick={onContinue} className="flex items-center justify-center gap-2 px-6 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-colors font-medium">Back to Library <ArrowRight className="w-4 h-4" /></button>
...
function BusinessOperationsPage({ user, onSignOut, onNavigate }: AuthedProps) {
  const [view, setView] = useState<ViewState>('overview')
  const [practiceScore, setPracticeScore] = useState(0)
  const [thisStatus, setThisStatus] = useState<TopicStatus>('not-started')
  useEffect(() => { loadTopicStatus(user.id, TOPIC_ID).then(setThisStatus) }, [user.id])
            ...
            <div className="space-y-3">
              <button onClick={() => setView('interactive-lesson')} className="w-full bg-white rounded-2xl border border-slate-200 p-4 flex items-center justify-between hover:border-slate-300 hover:shadow-md transition-all group text-left">
                <div className="flex items-center gap-4">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-lg font-bold shrink-0 ${thisStatus === 'mastered' ? 'bg-emerald-500 text-white' : 'bg-slate-900 text-white'}`}>{thisStatus === 'mastered' ? '✓' : '4'}</div>
...
export default withAuth(BusinessOperationsPage)
