                  ...
                  <div key={i} className="relative">
                    ...
                    <button onClick={() => setActiveBubble(isActive ? null : token)}
                      className={`px-4 py-2 rounded-lg font-mono text-xs font-semibold transition-all ${bubble
                        ? (isActive ? 'bg-blue-600 text-white shadow-md scale-105' : 'bg-blue-50 text-blue-700 border border-blue-200 hover:bg-blue-100')
        ...
        <motion.div key={q.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }}
          ...
          {!confirmed && selected !== null && (
            <div className="bg-amber-50 border border-amber-100 rounded-xl p-3 flex items-start gap-2">
              ...
              <p className="text-amber-700 text-xs">{q.hint}</p>
            </div>
          )}
          {confirmed && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              className={`rounded-xl p-3 ${selected === q.correctIndex ? 'bg-emerald-50 border border-emerald-100' : 'bg-red-50 border border-red-100'}`}>
...
function FeedbackModule({ score, total, onRetry, onContinue, nextTopicName }: {
  ...
  return (
    <div className="max-w-md mx-auto text-center space-y-6">
      <div className={`w-20 h-20 rounded-full flex items-center justify-center mx-auto ${mastered ? 'bg-emerald-100' : 'bg-amber-100'}`}>
        {mastered ? <Award className="w-8 h-8 text-emerald-600" /> : <AlertCircle className="w-8 h-8 text-amber-600" />}
      </div>
      <div>
      ...
      <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
        <motion.div initial={{ width: 0 }} animate={{ width: `${pct * 100}%` }} className={`h-full rounded-full ${mastered ? 'bg-emerald-500' : 'bg-amber-400'}`} />
      </div>
      <div className="flex flex-col gap-3">
        {mastered
          ? <button onClick={onContinue} className="flex items-center justify-center gap-2 px-6 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-colors font-medium">
...
async function loadTopicStatus(userId: string, topicId: string): Promise<TopicStatus> {
  const { data } = await supabase.from('study_progress').select('mastery_level')
    .eq('user_id', userId).eq('subject', SUBJECT).eq('grade', GRADE).eq('topic', topicId).single()
...
async function saveTopicStatus(userId: string, score: number, total: number) {
  ...
  await supabase.from('study_progress').upsert({
    user_id: userId, subject: SUBJECT, grade: GRADE, topic: TOPIC_ID,
    mastery_level: mastery, last_attempt_score: `${score}/${total}`,
    last_accessed: new Date().toISOString(), updated_at: new Date().toISOString(),
  }, { onConflict: 'user_id,subject,grade,topic' })
  return mastery as TopicStatus
}

function BusinessSectorsPage({ user, onSignOut, onNavigate }: AuthedProps) {
  ...
  useEffect(() => {
    loadTopicStatus(user.id, TOPIC_ID).then(setThisStatus)
    loadTopicStatus(user.id, NEXT_TOPIC_ID).then(setNextStatus)
  }, [user.id])

  const handlePracticeComplete = async (score: number) => {
    setPracticeScore(score)
    const st = await saveTopicStatus(user.id, score, TOPIC.initialQuestions.length)
    setThisStatus(st)
    if (score === 0) setView('remediation')
    else setView('feedback')
  }
  const handleRemediationComplete = async (score: number) => {
    await saveTopicStatus(user.id, score, TOPIC.remediationQuestions.length)
    setView('feedback')
  }
  const handleHardComplete = async (score: number) => {
    await saveTopicStatus(user.id, score, TOPIC.hardQuestions.length)
    setView('feedback')
  }
            ...
            <div className="space-y-3">
              <button onClick={() => setView('interactive-lesson')} className="w-full bg-white rounded-2xl border border-slate-200 p-4 flex items-center justify-between hover:border-slate-300 hover:shadow-md transition-all group text-left">
                <div className="flex items-center gap-4">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-lg font-bold shrink-0 ${thisStatus === 'mastered' ? 'bg-emerald-500 text-white' : 'bg-slate-900 text-white'}`}>
              ...
              <button onClick={() => onNavigate('learning-bizstudies-g10-t1-stakeholders' as AppPage)} className="w-full bg-white rounded-2xl border border-slate-200 p-4 flex items-center justify-between hover:border-slate-300 hover:shadow-md transition-all group text-left">
                <div className="flex items-center gap-4">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-lg font-bold shrink-0 ${nextStatus === 'mastered' ? 'bg-emerald-500 text-white' : 'bg-slate-900 text-white'}`}>
              ...
              {view === 'remediation' && (
                <motion.div key="remediation" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
                  <div className="bg-rose-50 border border-rose-200 rounded-2xl p-4 flex items-start gap-3">
                    ...
                    <div>
                      <p className="font-medium text-rose-900 text-sm">Let's try again</p>
                      <p className="text-rose-700 text-sm mt-0.5">Here are two more questions to build your understanding.</p>
                    </div>
                  </div>
              ...
              {view === 'feedback' && (
                <motion.div key="feedback" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  ...
                  <div className="mt-6 text-center">
                    <button onClick={() => setView('practice-more')} className="text-sm text-blue-600 hover:text-blue-800 transition-colors underline underline-offset-2">Want harder questions? Try Practice More</button>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
...
export default withAuth(BusinessSectorsPage)
