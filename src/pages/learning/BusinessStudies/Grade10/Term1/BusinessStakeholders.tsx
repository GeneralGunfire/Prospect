  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center justify-between text-sm text-slate-500">
        ...
        <div className="flex gap-1.5">{TOPIC.interactiveSteps.map((_, i) => <div key={i} className={`h-1.5 w-8 rounded-full transition-colors ${i <= step ? 'bg-blue-600' : 'bg-slate-200'}`} />)}</div>
      </div>
                ...
                return (
                  <div key={i} className="relative">
                    ...
                    <button onClick={() => setActiveBubble(isActive ? null : token)}
                      className={`px-4 py-2 rounded-lg font-mono text-xs font-semibold transition-all ${bubble ? (isActive ? 'bg-blue-600 text-white shadow-md scale-105' : 'bg-blue-50 text-blue-700 border border-blue-200 hover:bg-blue-100') : 'bg-white text-slate-400 border border-slate-200 cursor-default'}`}>
                      {token}
                    </button>
                  </div>
                )
      ...
      <div className="flex justify-between">
        ...
        {step < TOPIC.interactiveSteps.length - 1
          ...
          : <button onClick={onComplete} className="flex items-center gap-2 px-5 py-2.5 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-colors font-medium">Try a Worked Example <ArrowRight className="w-4 h-4" /></button>
        }
      </div>
    </div>
  )
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4">
        <div className="flex items-start gap-3">
          ...
          <div><p className="font-medium text-amber-900 text-sm mb-1">Worked Example</p><p className="text-amber-800 text-xs leading-relaxed">{TOPIC.guidedItem.problem}</p></div>
        </div>
      </div>
      <div className="flex gap-2 overflow-x-auto pb-1">
        {TOPIC.guidedItem.steps.map((s, i) => (
          <button key={s.id} onClick={() => { setStepIdx(i); setRevealed(false) }}
            className={`shrink-0 px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${i === stepIdx ? 'bg-slate-900 text-white' : i < stepIdx ? 'bg-blue-100 text-blue-700' : 'bg-slate-100 text-slate-500'}`}>
            Step {s.id}
          </button>
        ))}
      </div>
      <AnimatePresence mode="wait">
        <motion.div key={step.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="bg-white rounded-2xl border border-slate-200 p-4 space-y-3">
          ...
          <div className="bg-blue-50 border border-blue-100 rounded-xl px-5 py-4 text-xs font-semibold text-blue-900 text-center">{step.math}</div>
          {!revealed
      ...
      <div className="flex justify-between">
        ...
        {stepIdx < TOPIC.guidedItem.steps.length - 1
          ...
          : <button onClick={onComplete} className="flex items-center gap-2 px-5 py-2.5 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-colors font-medium">Start Practice <ArrowRight className="w-4 h-4" /></button>
        }
      </div>
    </div>
  )
  ...
  return (
    <div className="max-w-md mx-auto text-center space-y-6">
      ...
      <div className="flex flex-col gap-3">
        {mastered ? <button onClick={onContinue} className="flex items-center justify-center gap-2 px-6 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-colors font-medium">Next: {nextTopicName} <ArrowRight className="w-4 h-4" /></button>
...
function BusinessStakeholdersPage({ user, onSignOut, onNavigate }: AuthedProps) {
            ...
            <div className="space-y-3">
              <button onClick={() => setView('interactive-lesson')} className="w-full bg-white rounded-2xl border border-slate-200 p-4 flex items-center justify-between hover:border-slate-300 hover:shadow-md transition-all group text-left">
                <div className="flex items-center gap-4">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-lg font-bold shrink-0 ${thisStatus === 'mastered' ? 'bg-emerald-500 text-white' : 'bg-slate-900 text-white'}`}>{thisStatus === 'mastered' ? '✓' : '3'}</div>
              ...
              <button onClick={() => onNavigate('learning-bizstudies-g10-t1-operations' as AppPage)} className="w-full bg-white rounded-2xl border border-slate-200 p-4 flex items-center justify-between hover:border-slate-300 hover:shadow-md transition-all group text-left">
                <div className="flex items-center gap-4">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-lg font-bold shrink-0 ${nextStatus === 'mastered' ? 'bg-emerald-500 text-white' : 'bg-slate-900 text-white'}`}>{nextStatus === 'mastered' ? '✓' : '4'}</div>
        ...
        {view !== 'overview' && (
          <div className="space-y-6">
            ...
            <AnimatePresence mode="wait">
              ...
              {view === 'feedback' && <motion.div key="feedback" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><FeedbackModule score={practiceScore} total={TOPIC.initialQuestions.length} onRetry={() => setView('practice')} onContinue={() => onNavigate('learning-bizstudies-g10-t1-operations' as AppPage)} nextTopicName="Business Operations" /><div className="mt-6 text-center"><button onClick={() => setView('practice-more')} className="text-sm text-blue-600 hover:text-blue-800 transition-colors underline underline-offset-2">Want harder questions? Try Practice More</button></div></motion.div>}
            </AnimatePresence>
          </div>
        )}
      </main>
...
export default withAuth(BusinessStakeholdersPage)
