...
interface ScratchpadState {
  lines: { points: { x: number; y: number }[]; color: string; width: number }[];
  undoHistory: typeof lines[];
}
...
// ── main page ──────────────────────────────────────────────────────────────
function EconomicSystemsPage({ user, onSignOut, onNavigate }: AuthedProps) {
      ...
      await supabase.from('study_progress').upsert(
        {
          ...
          last_attempt_score: Math.round((score / total) * 100),
...
export default withAuth(EconomicSystemsPage);
