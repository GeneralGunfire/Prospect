          ...
          <div className="px-5 pt-5 pb-3 border-b border-slate-100 shrink-0">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                {math && <div className="overflow-x-auto mt-1 -mx-1 px-1"><span className="whitespace-nowrap font-mono text-sm font-black text-blue-700">{math}</span></div>}
              </div>
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
      ...
      <div className="flex flex-col items-center gap-3">
        ...
        <div className="w-full max-w-xs h-2 bg-slate-100 rounded-full overflow-hidden">
          <motion.div initial={{ width: 0 }} animate={{ width: `${pct}%` }} transition={{ duration: 0.8, delay: 0.3 }} className={`h-full rounded-full ${mastered ? 'bg-emerald-500' : 'bg-blue-500'}`} />
        </div>
      </div>
...
async function loadTopicProgress(userId: string): Promise<TopicStatus> {
  ...
  if (!data) return 'not-started'; const m = data.mastery_level as string
  if (m === 'mastered') return 'mastered'; if (m === 'needs_practice') return 'needs-practice'; return 'not-started'
}
...
function FiveKingdomsPage({ user, onNavigate }: AuthedProps) {
              ...
              <div className="space-y-4">
                <motion.div onClick={() => setView('interactive-lesson')}
                  ...
                  <div className="flex items-center gap-6">
                    <div className={`shrink-0 w-10 h-10 rounded-[1.25rem] flex items-center justify-center text-sm font-bold ${status === 'mastered' ? 'bg-emerald-500 text-white' : 'bg-slate-900 text-white shadow-md shadow-slate-900/10'}`}>{status === 'mastered' ? '✓' : '2'}</div>
                    <div>
                ...
                <motion.div onClick={() => onNavigate('learning-lifesci-g10-t1-taxonomy' as AppPage)}
                  className="bg-white rounded-xl border border-slate-200 p-4 flex items-center justify-between gap-6 cursor-pointer hover:border-slate-300 hover:shadow-md transition-all group">
                  <div className="flex items-center gap-6">
                    <div className={`shrink-0 w-10 h-10 rounded-[1.25rem] flex items-center justify-center text-sm font-bold ${nextStatus === 'mastered' ? 'bg-emerald-500 text-white' : 'bg-slate-900 text-white shadow-md shadow-slate-900/10'}`}>{nextStatus === 'mastered' ? '✓' : '3'}</div>
                    <div>
...
export default withAuth(FiveKingdomsPage)
