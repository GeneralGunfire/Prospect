          ...
          <div className="py-8 -mx-2 px-2">
            <div className="flex flex-wrap items-center justify-center gap-2 relative mx-auto">
              {step.math.map((char, i) => {
                ...
                return (<motion.div key={i} initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: i * 0.1 }} className="relative text-xl font-mono font-bold text-slate-900">{char}{bubble && <SpeechBubble text={bubble.text} pos={bubble.pos} />}</motion.div>)
              })}
            </div>
          </div>
        </motion.div>
...
const FeedbackModule = ({ correct, total, onRetry, onPracticeMore, onContinue, hidePracticeMore = false }: { correct: number; total: number; onRetry: () => void; onPracticeMore: () => void; onContinue: () => void; hidePracticeMore?: boolean }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-[2.5rem] border border-slate-200 p-4 text-center space-y-6 shadow-sm">
      ...
      <div><p className={`text-[10px] font-black uppercase tracking-widest mb-2 ${mastered ? 'text-emerald-600' : 'text-slate-400'}`}>{mastered ? '✦ Mastered' : 'Keep Practising'}</p><h2 className="text-xl font-bold text-slate-900 tracking-tighter">Your Results</h2><p className="text-sm text-slate-400 font-semibold mt-1">{TOPIC.title}</p></div>
...
function SpeciesConceptPage({ user, onNavigate }: AuthedProps) {
  ...
  useEffect(() => { if (!user) return; loadTopicProgress(user.id).then(s => setStatus(s)) }, [user])
              ...
              <div className="space-y-4">
                <motion.div onClick={() => setView('interactive-lesson')} className="bg-white rounded-xl border border-slate-200 p-4 flex items-center justify-between gap-6 cursor-pointer hover:border-slate-300 hover:shadow-md transition-all group">
                  <div className="flex items-center gap-6">
                    <div className={`shrink-0 w-10 h-10 rounded-[1.25rem] flex items-center justify-center text-sm font-bold ${status === 'mastered' ? 'bg-emerald-500 text-white' : 'bg-slate-900 text-white shadow-md shadow-slate-900/10'}`}>{status === 'mastered' ? '✓' : '4'}</div>
                    <div>
            ...
            <div className="max-w-2xl mx-auto">
              ...
              <AnimatePresence mode="wait">
                <motion.div key={view} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.25 }}>
                  ...
                  {view === 'remediation' && (
                    <div className="space-y-6">
                      <div className="bg-rose-50 border border-rose-200 rounded-xl p-3 flex gap-5 items-start"><AlertCircle className="text-rose-500 shrink-0 mt-0.5" size={24} /><div><p className="text-base font-black text-rose-900 uppercase tracking-tight mb-1">Let's Try Again</p><p className="text-rose-700 text-xs leading-relaxed">It's okay — let's work through two extra questions to build your confidence.</p></div></div>
...
export default withAuth(SpeciesConceptPage)
